...
## アプリ概要
目標設定を行い、日々の学習ログを蓄積。AIがデータ分析し、Keepを重視したKPT法(KeepとTryのみを表示し、Problemは表示しない)で目標ごとに週次レポートを作成する。
...
graph LR
  %% 画面遷移図
  A[ダッシュボード画面] --> B[目標詳細画面]
  B --> A

  %% 補足説明
	C[[表示内容: 今週の今日までの達成率、今週の今日までの総学習時間、今週の今日までの目標学習時間、目標一覧]] --> A
	D[[モーダルウィンドウ: 目標作成]] --> A
  E[[表示内容: 先週の週次レポート、累計時間、ログ一覧]] --> B
  F[[モーダルウィンドウ: 目標編集、ステータス変更]] --> B
  G[[モーダルウィンドウ: ログ作成・編集・削除]] --> B
```

## 画面一覧
|画面名|表示内容|
|-|-|
|ダッシュボード画面|上段: 今週の今日までの達成率 + 今週の今日までの総学習時間(分) + 今週の今日までの目標学習時間(分)<br>下段: 目標一覧|
|目標詳細画面|上段: 先週のレポート + 累計時間 ※右上に目標アーカイブボタン<br>下段: ログ作成ボタン、ログ一覧(ログ削除ボタン)|

## ディレクトリ
### 全体構成
```
log-lift/
├── frontend/ # フロントエンド（UI・画面管理）
...
```
frontend/
├── app/        # 画面（Next.js App Router / ページ・レイアウト）
├── components/ # 再利用可能なUIコンポーネント（ボタン・カードなど）
├── features/   # 機能単位のモジュール（goal, learning-logなど）
...
- 同名タイトルは許可
- 下記制御は、アプリケーション層で行う
	- ステータスがクローズ or アーカイブの場合、学習ログ登録および更新不可
